...
import webbrowser
webbrowser.open("https://t.me/SOFEK_7")
...
def info(email):
    ...
    try:
      ...
      #  print(data)
        ...
        tlg=f'''        
𝙜𝙢𝙖𝙞𝙡 :{email}
𝙧𝙚𝙨𝙚𝙩 : {instagram.Reset(username).get("data").get("reset")}
𝙣𝙖𝙢𝙚 : {nm}
𝙪𝙨𝙚𝙧𝙣𝙖𝙢𝙚 {username}
𝙛𝙤𝙡𝙡𝙤𝙬𝙞𝙣𝙜 : {folg}
𝙛𝙤𝙡𝙡𝙤𝙬𝙚𝙧𝙨 : {fol}
...
def gmail(email):
    ...
    if 'password' in response:
        info(email)
        ge+=1
        sys.stdout.write(f'''\r{F}Hits : {M}{ge}{Z} | Bad Ins: {M}{bi}{X} |Bad Em : {M}{be}{C} | Good Ins : {M}{gi}'''),sys.stdout.flush()
    else:
        be+=1
        sys.stdout.write(f'''\r{F}Hits : {M}{ge}{Z} | Bad Ins: {M}{bi}{X} |Bad Em : {M}{be}{C} | Good Ins : {M}{gi}'''),sys.stdout.flush()
